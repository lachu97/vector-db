...
class HNSWIndexer:
    def __init__(self, dim=384, space="cosine", max_elements=10000,
                 ef_construction=200, m=16, ef_query=50, index_path="index.bin"):
        self.dim = dim
        self.space = space
    ...
    def add_items(self, vectors: np.ndarray, ids: np.ndarray):
        ...
        with self.lock:
            ...
            if needed > 0:
                new_max = max(
                    self.index.get_max_elements() * 2,
                    self.index.get_current_count() + len(ids),
                )
                self.index.resize_index(new_max)
